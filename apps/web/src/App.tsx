...
import type { CandidateReview, Capabilities, CaseResult, Comparison, DiscoveryAnalysis, Evidence, Explanation, GitHubImportResult, PolicySummary, Report, Run, RunEvent, SpecSummary, Target, Verdict } from './types'
...
type IconName = 'home' | 'radar' | 'shield' | 'pulse' | 'compare' | 'report' | 'chevron' | 'refresh' | 'lock' | 'alert' | 'github'
...
function Icon({ name, size = 17 }: { name: IconName; size?: number }) {
  const paths: Record<IconName, ReactNode> = {
    ...
    github: <><path d="M12 2.8a9.2 9.2 0 0 0-2.9 17.9c.5.1.6-.2.6-.5v-1.8c-2.8.6-3.4-1.2-3.4-1.2-.5-1.2-1.1-1.5-1.1-1.5-.9-.6.1-.6.1-.6 1 0 1.6 1.1 1.6 1.1.9 1.6 2.4 1.1 2.9.8.1-.7.4-1.1.7-1.3-2.2-.3-4.6-1.1-4.6-4.7 0-1 .4-1.9 1-2.5-.1-.3-.4-1.3.1-2.6 0 0 .8-.3 2.7 1a9.3 9.3 0 0 1 4.9 0c1.9-1.3 2.7-1 2.7-1 .5 1.3.2 2.3.1 2.6.6.6 1 1.5 1 2.5 0 3.6-2.4 4.4-4.6 4.7.4.3.7.9.7 1.8v2.6c0 .4.2.6.7.5A9.2 9.2 0 0 0 12 2.8Z"/></>,
  }
...
function BoundaryModeGuide({ active }: { active: 'discover' | 'verify' }) {
  return <section className="mode-guide" aria-label="Discover and Verify difference">
    <article className={active === 'discover' ? 'active' : ''}>
      <span className="mode-number">01</span>
      <div><p className="eyebrow">Discover · read the map</p><h2>Where could access break?</h2><p>Reads OpenAPI and optional HAR. Finds sensitive-looking routes and policy candidates. <strong>No request is sent to the target API.</strong></p></div>
      <span className="mode-output">Output: risk map</span>
    </article>
    <span className="mode-arrow" aria-hidden="true">→</span>
    <article className={active === 'verify' ? 'active' : ''}>
      <span className="mode-number">03</span>
      <div><p className="eyebrow">Verify · test the door</p><h2>Does access really behave?</h2><p>Uses approved policy, identities and an allowlisted target. <strong>Sends bounded requests and saves redacted proof.</strong></p></div>
      <span className="mode-output">Output: verdict + evidence</span>
    </article>
  </section>
}

const discoveryStages = [
  { title: 'Validate source', detail: 'Parse OpenAPI safely and enforce analysis limits.' },
  { title: 'Inventory operations', detail: 'Read methods, paths, schemas and authentication hints.' },
  { title: 'Infer access risks', detail: 'Find resource IDs and propose ownership-sensitive rules.' },
  { title: 'Compare traffic', detail: 'Match optional HAR routes without keeping bodies or secrets.' },
]
...
function MentorOverview({ setView, runs, targets, capabilities }: { setView: (view: View) => void; runs: Run[]; targets: Target[]; capabilities: Capabilities | null }) {
  ...
  return <>
    <section className="mentor-hero">
      <div><p className="eyebrow light">Authorization release gate</p><h1>Prove revoked access<br/><span>stays revoked.</span></h1><p>Import a real API contract, replay the permission journey, and block a release when protected data still escapes.</p><div className="mentor-actions"><button className="button primary" onClick={() => setView('discovery')}><Icon name="github" size={15}/> Connect a repository</button><button className="button dark-secondary" onClick={() => setView('runs')}><Icon name="pulse" size={15}/> Run verification</button></div></div>
    ...
    <section className="live-proof-strip" aria-label="Current workspace proof"><div><span>Saved runs</span><strong>{completed.length}</strong></div><div className={violationCount ? 'danger' : ''}><span>Violations found</span><strong>{violationCount}</strong></div><div><span>Execution</span><strong>{targets.some(target => !target.synthetic_fixture) ? 'Staging ready' : 'Local lab ready'}</strong></div><div><span>GitHub source</span><strong>{capabilities?.sources.github ? 'Repository import ready' : 'Unavailable'}</strong></div></section>
    <section className="quick-path"><article className="card"><span>01</span><div><p className="eyebrow">Use real project data</p><h2>Connect GitHub</h2><p>Import an OpenAPI JSON contract from a public or server-authorized private repository.</p></div><button className="button secondary" onClick={() => setView('discovery')}>Open repository import <Icon name="chevron" size={13}/></button></article><article className="card"><span>02</span><div><p className="eyebrow">Generate real evidence</p><h2>Run the permission test</h2><p>Watch backend worker events, HTTP results and redacted evidence update live.</p></div><button className="button secondary" onClick={() => setView('runs')}>Open verification <Icon name="chevron" size={13}/></button></article><article className="card"><span>03</span><div><p className="eyebrow">Make a release decision</p><h2>Compare the repair</h2><p>Reject leaks, incomplete evidence and fixes that break valid access.</p></div><button className="button secondary" onClick={() => setView('compare')}>Open release gate <Icon name="chevron" size={13}/></button></article></section>
    <section className="trust-line"><span><Icon name="lock"/>Allowlisted targets</span><span><Icon name="shield"/>HttpOnly + CSRF</span><span><Icon name="pulse"/>Bounded execution</span><span><Icon name="report"/>Redacted evidence</span></section>
  </>
}
...
function RunActivity({ run }: { run: Run }) {
  ...
  useEffect(() => {
    ...
    setEvents([]); setError('')
    if (apiStatus.connection === 'offline') return () => { disposed = true }
    void load()
    if (!active) return () => { disposed = true }
    const timer = window.setInterval(() => void load(), 700)
    return () => { disposed = true; window.clearInterval(timer) }
  }, [run.id, active, apiStatus.connection])
...
function RunsView({ targets, targetAlias, onTargetChange, runs, selectedRun, select, refresh, capabilities }: { targets: Target[]; targetAlias: string; onTargetChange: (alias: string) => void; runs: Run[]; selectedRun: Run | null; select: (run: Run) => Promise<void>; refresh: () => Promise<void>; capabilities: Capabilities | null }) {
  ...
  async function start(alias: string) { setBusy(true); setError(''); setOutcome(''); setActivity(`Creating a bounded run for ${alias.replace('demo-','')}…`); try { const run = await api.startRun(alias); setActivity('Run accepted. Loading its live backend trace…'); await refresh(); await select(run); setOutcome(`Run ${run.id.slice(-6)} accepted. Backend execution is visible below.`) } catch (e) { setError(e instanceof Error ? e.message : 'Could not start run') } finally { setBusy(false); setActivity('') } }
  async function matrix() { setBusy(true); setError(''); setOutcome(''); try { let first: Run | null = null; for (const [index, item] of labTargets.entries()) { setActivity(`Queueing build ${index + 1} of ${labTargets.length}: ${item.label}`); const run = await api.startRun(item.alias); first ||= run } await refresh(); if (first) await select(first); setOutcome(`${labTargets.length} builds queued. The worker will verify them one at a time.`) } catch (e) { setError(e instanceof Error ? e.message : 'Could not queue lab matrix') } finally { setBusy(false); setActivity('') } }
  ...
  return <><header className="page-head"><div><p className="eyebrow">Runtime verification</p><h1>{noTargets ? 'Connect a target to begin.' : realMode ? 'Test a real access boundary.' : 'Prove access stays revoked.'}</h1><p>{noTargets ? 'Active requests remain disabled until a reviewed staging registry or explicit lab mode is loaded.' : realMode ? 'Send one bounded GET per reviewed identity and keep only redacted proof.' : 'Replay the disclosed sharing lifecycle and see exactly where the permission boundary holds or breaks.'}</p></div><div className="demo-flag"><i/>{realMode ? 'Authorized staging' : targets.length ? 'Disclosed lab' : 'No target configured'}</div></header>
    <BoundaryModeGuide active="verify"/>
    ...
    <section className="verification-flow card" aria-label="What happens during verification"><div className="flow-intro"><p className="eyebrow">What this button will do</p><h2>One controlled test, four visible stages</h2><p>The live trace below updates as the backend moves through each stage.</p></div><ol><li><span>1</span><div><strong>Load policy</strong><small>Freeze expected access rules</small></div></li><li><span>2</span><div><strong>Prepare identities</strong><small>Owner, collaborator and outsider</small></div></li><li><span>3</span><div><strong>Send requests</strong><small>Grant, use, revoke and retrieve</small></div></li><li><span>4</span><div><strong>Seal evidence</strong><small>Pass, violation or inconclusive</small></div></li></ol></section>
    ...
    <section className="launch card"><div className="launch-copy"><span className="launch-glyph"><Icon name="pulse" size={21}/></span><div><h2>Start a live verification</h2><p>{realMode ? `Fixed read probe through ${selectedTarget?.origin}` : 'Choose a disclosed lab build. The runner will send real HTTP requests inside the local fixture.'}</p><div className="assurance-row"><span><Icon name="lock" size={13}/>Allowlisted</span><span>{selectedTarget?.limits.requests || 0} requests max</span><span>1 at a time</span><span>No redirects</span></div><p className="expected-result"><strong>You will see:</strong> worker trace → case verdicts → redacted evidence.</p></div></div><div className="launch-actions"><label htmlFor="target">Which implementation should be tested?</label><select id="target" value={targetAlias} onChange={e => { setOutcome(''); onTargetChange(e.target.value) }}>{targets.map(item => <option value={item.alias} key={item.alias} disabled={!item.ready}>{item.label}{item.ready ? '' : ' · missing environment'}</option>)}</select><button className="button primary" onClick={() => selectedTarget && start(selectedTarget.alias)} disabled={busy || !selectedTarget?.ready}>{busy ? <><span className="button-spinner"/>Starting worker…</> : 'Start live verification'}</button>{labTargets.length > 1 && <button className="button secondary" onClick={matrix} disabled={busy}>Verify all 3 lab builds</button>}</div>{selectedTarget && !selectedTarget.ready && <p className="form-error full" role="alert">Missing runtime environment references: {selectedTarget.missing_environment.join(', ')}</p>}{error && <p className="form-error full" role="alert">{error}</p>}{outcome && <div className="action-feedback action-success full" role="status"><span className="feedback-symbol">✓</span><div><strong>Verification started</strong><span>{outcome}</span></div></div>}</section>}
...
function DiscoveryView({ spec, capabilities }: { spec: SpecSummary | null; capabilities: Capabilities | null }) {
  ...
  const [error, setError] = useState('')
  const [githubRepository, setGitHubRepository] = useState('ASHMITKATHAT/SentinelAPI-BoundaryLab')
  const [githubRef, setGitHubRef] = useState('main')
  const [githubPath, setGitHubPath] = useState('contracts/control-api.openapi.json')
  ...
  useEffect(() => {
    if (!busy) return
    const timer = window.setInterval(() => setAnalysisStep(current => Math.min(current + 1, discoveryStages.length - 1)), 360)
  ...
  async function importFromGitHub() {
    setGitHubBusy(true); setGitHubResult(null); setResult(null); setError('')
    try {
      const imported = await api.importGitHubOpenApi(githubRepository, githubRef, githubPath)
      setGitHubResult(imported)
      setSpecText(JSON.stringify(imported.document, null, 2))
      setLabel(`${imported.repository} · ${imported.path}`)
    } catch (caught) { setError(caught instanceof Error ? caught.message : 'GitHub contract import failed') }
    finally { setGitHubBusy(false) }
  }
  async function analyze() {
    setBusy(true); setResult(null); setError(''); setAnalysisStep(0); setAnalysisDuration(null)
    ...
    try {
      ...
      const engineDuration = Math.max(1, Math.round(performance.now() - startedAt))
      const minimumPresentationTime = 1500
      const remainingTime = minimumPresentationTime - (performance.now() - startedAt)
      if (remainingTime > 0) await new Promise(resolve => window.setTimeout(resolve, remainingTime))
      setAnalysisStep(3); setAnalysisDuration(engineDuration)
  ...
  return <><header className="page-head"><div><p className="eyebrow">Repository discovery</p><h1>Connect the API contract.</h1><p>Import OpenAPI from GitHub or upload it manually, then map ownership-sensitive routes without sending requests to the target API.</p></div><div className="demo-flag"><i/>Passive · no target requests</div></header>
    <BoundaryModeGuide active="discover"/>
    <section className="card github-import"><div className="github-intro"><span className="github-mark"><Icon name="github" size={24}/></span><div><p className="eyebrow">Real repository source</p><h2>Import OpenAPI from GitHub</h2><p>Public repositories work directly. Private repository tokens stay in the server environment and are never entered in this browser.</p></div><span className={`integration-badge ${capabilities?.sources.github_private_access ? 'private-ready' : ''}`}>{capabilities?.sources.github_private_access ? 'Private access configured' : 'Public access ready'}</span></div><div className="github-fields"><label>Repository<input value={githubRepository} onChange={event => { setGitHubRepository(event.target.value); setGitHubResult(null); setError('') }} placeholder="owner/repository"/></label><label>Branch or ref<input value={githubRef} onChange={event => { setGitHubRef(event.target.value); setGitHubResult(null); setError('') }} placeholder="default branch"/></label><label>OpenAPI JSON path<input value={githubPath} onChange={event => { setGitHubPath(event.target.value); setGitHubResult(null); setError('') }} placeholder="contracts/openapi.json"/></label><button className="button github-button" disabled={githubBusy || !githubRepository.trim() || !githubPath.trim()} onClick={importFromGitHub}>{githubBusy ? <><span className="button-spinner"/>Importing contract…</> : <><Icon name="github" size={15}/>Connect & import</>}</button></div>{githubBusy && <div className="action-feedback action-running" role="status"><div className="spinner"/><div><strong>Connecting to GitHub</strong><span>Reading repository metadata and the selected contract through the server.</span></div></div>}{githubResult && <div className="action-feedback action-success" role="status"><span className="feedback-symbol">✓</span><div><strong>Repository contract loaded</strong><span>{githubResult.repository}@{githubResult.ref} · {githubResult.path} · {githubResult.operation_count} operations · SHA {githubResult.file_sha.slice(0,12)}…</span></div><a href={githubResult.repository_url} target="_blank" rel="noreferrer">View repository</a></div>}</section>
    <section className="card discovery-input"><div className="discovery-copy"><div><h2>Review the imported source</h2><p>Upload a local contract instead if needed. Add HAR only for documented-versus-observed route analysis.</p></div><div className="input-safety"><Icon name="shield"/><span><strong>Human approval required</strong><small>Candidates never enter policy automatically</small></span></div></div><div className="import-grid"><div className={`source-card ${specText ? 'source-ready' : ''}`}><div className="source-title"><span className="source-icon"><Icon name="report"/></span><span><b>OpenAPI contract</b><small>JSON · required</small></span><em>{specText ? 'Ready' : 'Required'}</em></div><p className="source-help">Operations, schemas and auth schemes</p><div className="source-actions"><label className="button secondary compact" htmlFor="openapi-file">Choose local file</label><input id="openapi-file" className="visually-hidden" type="file" accept="application/json,.json" onChange={event => loadFile(event.target.files?.[0], 'spec')}/><span>{specText ? 'Contract loaded' : 'No contract selected'}</span></div><details className="source-details"><summary>Inspect JSON</summary><textarea value={specText} onChange={event => { setSpecText(event.target.value); setGitHubResult(null); setError('') }} spellCheck={false} aria-label="OpenAPI JSON source"/></details></div><div className={`source-card ${harText ? 'source-ready' : ''}`}><div className="source-title"><span className="source-icon"><Icon name="radar"/></span><span><b>Traffic sample</b><small>HAR · optional</small></span><em className="optional">{harText ? 'Ready' : 'Optional'}</em></div><p className="source-help">Route diff only; secrets and bodies are discarded</p><div className="source-actions"><label className="button secondary compact" htmlFor="har-file">Choose HAR</label><input id="har-file" className="visually-hidden" type="file" accept="application/json,.har" onChange={event => loadFile(event.target.files?.[0], 'har')}/><button className="text-link" type="button" onClick={loadDemoTraffic}>Use demo traffic</button></div><details className="source-details"><summary>Inspect HAR</summary><textarea value={harText} onChange={event => { setHarText(event.target.value); setError('') }} placeholder="Paste HAR JSON" spellCheck={false} aria-label="HAR JSON source"/></details></div></div><details className="analysis-options"><summary>Analysis label and limits</summary><label className="analysis-name">Analysis label<input value={label} maxLength={120} onChange={event => { setLabel(event.target.value); setError('') }}/></label><div className="limit-row"><span>2 MB max</span><span>500 paths</span><span>5,000 HAR entries</span></div></details><div className="discovery-submit"><button className="button primary" disabled={busy || !label.trim() || !specText.trim()} onClick={analyze}>{busy ? <><span className="button-spinner"/>Mapping boundary…</> : 'Map API boundary'}</button><span className="submit-note"><strong>Click result:</strong> validates the contract, inventories routes, proposes review rules and saves the analysis.</span></div>{busy && <section className="analysis-progress" role="status" aria-live="polite"><div className="analysis-progress-head"><div className="spinner"/><div><p className="eyebrow">Passive analysis in progress</p><h3>{discoveryStages[analysisStep].title}</h3><p>{discoveryStages[analysisStep].detail}</p></div><strong>{Math.min(90, (analysisStep + 1) * 25)}%</strong></div><div className="analysis-track"><i style={{ width: `${Math.min(90, (analysisStep + 1) * 25)}%` }}/></div><ol>{discoveryStages.map((stage, index) => <li key={stage.title} className={index < analysisStep ? 'complete' : index === analysisStep ? 'current' : ''}><span>{index < analysisStep ? '✓' : index + 1}</span><div><strong>{stage.title}</strong><small>{stage.detail}</small></div></li>)}</ol><div className="passive-note"><Icon name="shield" size={14}/><span>This step only reads supplied files. It does not call the target API.</span></div></section>}{result && <section className="analysis-complete" role="status"><div className="analysis-complete-head"><span className="feedback-symbol">✓</span><div><p className="eyebrow">Boundary mapped</p><h3>Here is exactly what happened</h3><p>Engine completed in {analysisDuration ?? 0} ms. The result was saved as analysis <code>{result.id.slice(-8)}</code>.</p></div></div><div className="analysis-outcomes"><article><span>1</span><div><strong>Contract read</strong><small>{result.summary.documented_operations} operations inventoried</small></div></article><article><span>2</span><div><strong>Review rules proposed</strong><small>{result.summary.ownership_candidates} ownership candidates; none auto-approved</small></div></article><article><span>3</span><div><strong>{result.traffic_diff.provided ? 'Traffic compared' : 'Traffic skipped'}</strong><small>{result.traffic_diff.provided ? `${result.summary.shadow_operations} unmatched routes found` : 'Add HAR only if route drift matters'}</small></div></article><article><span>4</span><div><strong>No target called</strong><small>Move to Verify when a rule is approved</small></div></article></div></section>}{error && <p className="form-error" role="alert">{error}</p>}</section>
...
function ReportsView({ runs }: { runs: Run[] }) {
  ...
  async function download(run: Run, format: 'report_html'|'results_json') { const key=run.id+format; setBusy(key); setMessage(''); try { const artifact=await api.artifact(run.id,format); window.location.assign(artifact.download_path); setMessage(`Downloaded ${format === 'report_html' ? 'HTML report' : 'JSON evidence'} · SHA-256 ${artifact.sha256.slice(0,16)}…`) } catch(e){ setMessage(e instanceof Error?e.message:'Export failed') } finally{setBusy('')} }
  return <><header className="page-head"><div><p className="eyebrow">Evidence handoff</p><h1>Share the proof.</h1><p>Export the tested cases, build, policy and incomplete context in one reviewable artifact.</p></div></header>
    <section className="report-list">{completed.map(run => <article className="card report-row" key={run.id}><div className={`report-mark ${run.assessment === 'blocked' ? 'danger' : ''}`}>{run.assessment === 'blocked' ? '!' : '✓'}</div><div><p className="eyebrow">{run.id}</p><h2>{run.target_alias.replace('demo-','')}</h2><p>{run.counts.pass} pass · {run.counts.violation} violations · {run.counts.inconclusive} inconclusive · {run.request_count} requests</p></div><StatusBadge value={run.assessment}/><div className="report-actions"><button className="button secondary compact" disabled={!!busy} onClick={()=>download(run,'report_html')}>{busy === run.id+'report_html' ? 'Preparing HTML…' : 'Download HTML'}</button><button className="button secondary compact" disabled={!!busy} onClick={()=>download(run,'results_json')}>{busy === run.id+'results_json' ? 'Preparing JSON…' : 'Download JSON'}</button></div></article>)}{!completed.length && <section className="empty-state"><h2>Complete a run first</h2><p>Reports are generated only from persisted evidence.</p></section>}</section>{message && <div className="toast" role="status">{message}</div>}
  </>
}
...
function WorkbenchApp() {
  ...
  useEffect(() => { if (!active || auth !== 'ready' || apiStatus.connection === 'offline') return; const timer=window.setInterval(() => loadRuns().catch(()=>undefined),1000); return()=>window.clearInterval(timer) }, [active, auth, apiStatus.connection, loadRuns])
  ...
  return <div className="app-shell"><SideNav view={view} setView={setView} logout={logout} activeRuns={activeRuns}/><main className="content">
    ...
    {apiStatus.connection === 'offline' && <section className="connection-banner" role="alert"><Icon name="alert"/><div><strong>BoundaryLab server is stopped</strong><span>Start it with <code>.\tools\start-local.ps1 -LabFixtures</code>, then reconnect. Automatic polling is paused while offline.</span></div><button className="button secondary compact" disabled={workspaceLoading} onClick={() => void reconnect()}>{workspaceLoading ? 'Checking…' : 'Reconnect'}</button></section>}
    ...
    {!workspaceLoading && !workspaceError && <Fragment key={workspaceRevision}>
      ...
      {view === 'discovery' && <DiscoveryView spec={spec} capabilities={capabilities}/>}
